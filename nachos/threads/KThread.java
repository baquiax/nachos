...
public class KThread {
    ...
    private static class NOP implements Runnable {
        ...
        public void run() {
            try {
                Thread.sleep(5000);
                Lib.debug('j', "NOP: I start to count");
                Lib.debug(dbgCommunication, "I finish to count");
                Lib.debug('j', "NOP: I finish to count");
                currentThread.yield();
                Thread.sleep(1000);    
            } catch (Exception e) {
    ...
    private static class ListenerTest implements Runnable {
        ...
        public void run() {                    
            ...
            int messageReceived = this.communicator.listen();
            Lib.debug(dbgCommunication, "LISTENER: I have received the message: \"" + messageReceived + "\"");
        }
    }
    ...
    private static class JoinAndWaitUntilTest implements Runnable {
        ...
        public void run() {
            for (int i=0; i< 10; i++) {                
                ...
                Lib.debug(dbgJoin, "THREAD #" + which + ": current value: "+ i);
                currentThread.yield();            
            }
            Lib.debug(dbgWaitUntil, "THREAD #" + which + " FINISHED!");
        }
    }
}
